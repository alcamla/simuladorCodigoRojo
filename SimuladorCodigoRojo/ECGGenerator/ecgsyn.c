...
double hrmean, double hrstd, double sf, int n)
{
   ...
   df = sf/n;
   for(i=1;i<=n;i++) w[i] = (i-1)*2.0*M_PI*df;
   for(i=1;i<=n;i++) 
   {
      dw1 = w[i]-w1;
      dw2 = w[i]-w2;
      Hw[i] = sig1*exp(-dw1*dw1/(2.0*c1*c1))/sqrt(2*M_PI*c1*c1)
            + sig2*exp(-dw2*dw2/(2.0*c2*c2))/sqrt(2*M_PI*c2*c2);
   }
    for(i=1;i<=n/2;i++){
...
void derivspqrst(double t0,double x[],double dxdt[])
{
   ...
   zbase = 0.005*sin(2.0*M_PI*fhi*t0);
...
void detectpeaks(double *ipeak, double *x, double *y, double *z, int n)
{
